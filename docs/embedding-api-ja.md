...
`reflect` / `bindAuto` で公開した D class は、`to!T()` や `bindFunc` のクラス引数へ戻すと
元インスタンスと `is` で一致する参照になります。通常の関数呼び出し、UFCS、D 関数の
戻り値を別の D 関数へ渡す場合も同じです。`bindType` の登録は逆変換の必須条件ではありません。
実際の D オブジェクトへの実行時キャストで判定するため、派生型から基底型への変換と、
基底型として公開された派生オブジェクトの派生型への変換に対応します。無関係なクラス、
通常のテーブル、型名や型チェーンだけを設定したテーブルはクラス引数に変換できません。
`{ ...instance }` によるテーブルコピーにも D オブジェクトの参照は引き継がれません。

クラス引数のオーバーロードは、実オブジェクトの型との完全一致を基底型への変換より優先します。
D の null クラス参照は Dua の `null` になり、Dua の `null` は D の null クラス参照へ戻ります。
`null` はクラスの型情報を持たないため、クラス引数だけが異なる複数の候補では曖昧エラーになります。
このホスト境界の null 変換は、スクリプトの `cast(Type)` や Optional 型の既存規則を変更しません。
